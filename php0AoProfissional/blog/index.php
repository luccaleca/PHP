...
echo helper::resumirTexto($texto, 100, 'continue');
echo "<hr>";
...
echo helper::formatarValor(50000000);
echo "<hr>";

echo helper::formatarNumero(10000);

echo "<hr>";

echo helper::contarTempo('2024-01-01 15:16:15');

$url = 'https://unset.';

var_dump(helper::validarUrl ($url));
echo '<hr>';
var_dump(helper::validarUrlComFiltro($url));
...
echo '<br>';
echo  helper::saudacao() .' Hoje é ' . helper::dataAtual();
echo '<hr>';

echo helper::slug('Esse texto é uma string pensanda em se tornar uma url. A idéia é essa.');
